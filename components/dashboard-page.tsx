  ...
  return (
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6" aria-label="Résumé global">
      {metrics.map(([label, value, note, Icon, iconColor]) => (
        <div className="flex min-h-24 items-center gap-3 rounded-[10px] border border-[#dbe3eb] bg-white px-4 py-4 shadow-[0_1px_2px_rgba(15,23,42,.06),0_5px_14px_rgba(15,23,42,.045)] transition-all duration-200 hover:-translate-y-0.5 hover:border-accent-400 hover:shadow-[0_3px_6px_rgba(15,23,42,.08),0_10px_22px_rgba(15,23,42,.08)]" key={label}>
...
function AnalyticsPanel({ icon: Icon, eyebrow, title, children }: { icon: LucideIcon; eyebrow: string; title: string; children: React.ReactNode }) {
  return <section className="rounded-[10px] border border-[#dbe3eb] bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,.06),0_5px_14px_rgba(15,23,42,.045)] transition-all duration-200 hover:-translate-y-0.5 hover:border-accent-400 hover:shadow-[0_3px_6px_rgba(15,23,42,.08),0_10px_22px_rgba(15,23,42,.08)]"><div className="mb-5 flex items-start gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-panel text-brand-900"><Icon size={14} /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-accent-400">{eyebrow}</p><h2 className="mt-1 font-display text-lg font-bold tracking-tight text-ink">{title}</h2></div></div><div className="space-y-4">{children}</div></section>;
}
